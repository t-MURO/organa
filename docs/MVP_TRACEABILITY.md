...
| 14 | Active-device sync and offline recovery | Partial | Encrypted transactional outbox, strictly monotonic serialized client field timestamps, ordered remote delivery, pending-record hydration guards, acknowledgement repulls, private Broadcast, paginated pulls, durable reconciliation, and local two-origin sync | Measure connected two-client latency and missed-broadcast/offline recovery |
