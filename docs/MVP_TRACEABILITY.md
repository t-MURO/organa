...
| 17 | Installable PWA with supported offline behavior | Local + managed preview | Manifest/icons, Workbox configuration, external service-worker bootstrap, 27 artifact/CSP/header checks, 23 precached URLs, update drill, signed-in offline drill, and 16 live EAS response checks | Production promotion and the release-browser matrix remain open; Web Push remains criterion 5 evidence |
