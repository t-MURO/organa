...
| 2 | Recovery-key confirmation and trusted-device enrollment | Partial | `apps/mobile/src/security/security-boundary.tsx`, strict native/web stored-identity parsing, server-resolved missing/untrusted/revoked cleanup, `packages/crypto/src/recovery-key.ts`, `packages/crypto/src/device-approval.ts`, local two-origin approval drill | Repeat enrollment, approval, rejection, expiry, malformed/missing identity, and recovery on connected clean clients |
...
| 11 | Continuous searchable offline Brain Dump with safe multi-device editing | Partial | `apps/mobile/src/features/brain-dump`, encrypted Yjs updates, exact-set compaction, local merge/offline evidence, and atomic cleanup/rejection for deleted current structured bullets | Connected two-client concurrent editing, delete-versus-update, missed-broadcast recovery, and compaction observation |
...
| 18 | Data export and one-hour account deletion | Partial | `apps/mobile/src/features/account`, readable/encrypted exports, restore validation, deletion migration, local finalizer drill, and the guarded `verify:connected:acceptance:deletion` operator command | Pass the real connected one-hour scheduler drill and restore an encrypted export on a separate clean device |
