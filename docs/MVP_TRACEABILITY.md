...
| 14 | Active-device sync and offline recovery | Partial | Encrypted transactional outbox, monotonic field timestamps, server-version acknowledgements, hydration/revision guards, private Broadcast, paginated pulls, durable reconciliation, local two-origin sync, and managed two-session evidence for sub-second delivery plus missed-broadcast recovery | Complete rendered offline/network-transition and large-account drills, then repeat against production |
