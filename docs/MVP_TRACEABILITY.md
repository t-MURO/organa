...
Status audited on 2026-07-24 against `REQUIREMENTS.md` section 19.
...
| 2 | Recovery-key confirmation and trusted-device enrollment | Partial | `apps/mobile/src/security/security-boundary.tsx`, `packages/crypto/src/recovery-key.ts`, `packages/crypto/src/device-approval.ts`, local two-origin approval drill | Repeat enrollment, approval, rejection, expiry, and recovery on connected clean clients |
...
| 5 | Multiple reminders, snooze presets, and primary reminder device | Partial | Native/web schedulers, Focus presets, reminder authorization cache, private device RPCs, and local Supabase checks | Connected ownership/revocation drill plus physical notification/action/snooze delivery |
...
| 11 | Continuous searchable offline Brain Dump with safe multi-device editing | Partial | `apps/mobile/src/features/brain-dump`, encrypted Yjs updates, exact-set compaction, local merge/offline evidence | Connected two-client concurrent editing, missed-broadcast recovery, and compaction observation |
...
| 13 | Offline usability after initial sign-in | Partial | SQLite/IndexedDB repositories and a signed-in production-PWA offline/reconnect drill | Physical native process-termination and offline reminder behavior |
| 14 | Active-device sync and offline recovery | Partial | Encrypted outbox, private Broadcast, paginated pulls, durable reconciliation, and local two-origin sync | Measure connected two-client latency and missed-broadcast/offline recovery |
| 15 | End-to-end encryption with no backend plaintext | Partial | `packages/crypto/src/record-encryption.ts`, account-scoped key vaults, ciphertext-only local Supabase rows, and schema/RLS contracts | Connected-deployment ciphertext inspection and independent security review |
| 16 | Light, dark, system, and manual themes | Local | `apps/mobile/src/theme.ts`, settings context, browser walkthrough, and documented AA token contrast | None for local product behavior |
...
| 18 | Data export and one-hour account deletion | Partial | `apps/mobile/src/features/account`, readable/encrypted exports, restore validation, deletion migration, local finalizer drill, and the guarded `verify:connected:deletion` operator command | Pass the real connected one-hour scheduler drill and restore an encrypted export on a separate clean device |
