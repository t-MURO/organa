...
| 11 | Continuous searchable offline Brain Dump with safe multi-device editing | Partial | `apps/mobile/src/features/brain-dump`, one-submit-per-platform Enter handling, encrypted Yjs updates, exact-set compaction, local merge/offline evidence, and atomic cleanup/rejection for deleted current structured bullets | Connected two-client concurrent editing, delete-versus-update, missed-broadcast recovery, and compaction observation |
