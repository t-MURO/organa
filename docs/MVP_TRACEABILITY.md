...
| 2 | Recovery-key confirmation and trusted-device enrollment | Partial | `apps/mobile/src/security/security-boundary.tsx`, strict native/web stored-identity parsing, a connected untrusted-device erasure/recovery-boundary drill, `packages/crypto/src/recovery-key.ts`, `packages/crypto/src/device-approval.ts`, and the local two-origin approval drill | Repeat enrollment, approval, rejection, expiry, malformed/missing identity, and recovery on home-server and production clean clients |
...
| 11 | Continuous searchable offline Brain Dump with safe multi-device editing | Partial | `apps/mobile/src/features/brain-dump`, a direct one-Enter/one-new-bullet browser drill, platform-separated submit handling, encrypted Yjs updates, exact-set compaction, local merge/offline evidence, and atomic cleanup/rejection for deleted current structured bullets | Connected two-client concurrent editing, delete-versus-update, missed-broadcast recovery, compaction observation, and physical native keyboard confirmation |
