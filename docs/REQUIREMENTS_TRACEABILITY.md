...
| --- | --- | --- | --- |
| 1 | Google, GitHub, and email account creation | OAuth and email-code adapters plus the sign-in UI are in `apps/mobile/src/auth`; local email-code contracts and a real local OTP walkthrough pass. Native PKCE callback tests and contracts cover attached browser completion, app resume, cold start, exact redirect matching, safe errors, and one-time code deduplication. The managed test project has a live web Site URL plus an exact web/native/local allowlist. Maileroo custom SMTP and both six-digit/15-minute code-only email templates are configured and pass the read-only Management API verifier. A user-confirmed live preview walkthrough received and accepted the Maileroo-delivered code. Reported consumed-link callbacks produce a fixed code-recovery message and clean the stale URL fragment. Provider discovery now validates managed Auth settings, fails closed, and renders only enabled OAuth methods without delaying an authenticated offline session; the current email-only managed state passed a production-build browser walkthrough. A private-file managed OAuth provisioner now applies or checks Google/GitHub state without printing credentials. | Connected gate: issue provider-console credentials, run the guarded provisioner, disable Maileroo tracking before link-based security flows, exercise Google and GitHub on the connected test deployment, then replace the test Site URL and repeat every provider against production. |
| 2 | Recovery confirmation and trusted-device enrollment | `packages/crypto` checks cover `ORG1` recovery and target-bound `ODA1` transfer. Native/web identity parsing rejects malformed stored fields, and a successful device-list read erases local private state when the current identity is missing, untrusted, or revoked without weakening genuine offline access. A managed 119-check backend run directly exercised recovery proof, approval, claim, rejection, expiry, revocation, cross-account denial, and exact synthetic-user cleanup. | Physical separate-device recovery plus malformed/missing-identity drills remain; repeat against production. |
...
| 15 | End-to-end encrypted content inaccessible to backend | AES-256-GCM field envelopes, keyed opaque Check-In IDs, wrapped web keys, SecureStore native keys, protected browser session/device-proof storage, fail-closed client configuration, and explicit data/retention mapping are implemented. Managed checks directly inspected durable ciphertext, enforced cross-account RLS and proof-gated RPCs, restricted private Broadcasts to opaque hints plus Supabase protocol UUIDs, and removed an unallowlisted Push endpoint before outbound access. | Independent cryptographic/application review, production abuse/operations repeat, and legal validation of store declarations. |
...
| 20 | Critical/high security findings resolved | Security design, local abuse verifiers, explicit revocation of direct access to internal database functions, targeted dependency inspection, exact-hash script CSP, generated host-header/cache policy, and a live deployment verifier are documented. The internal audit resolved device-migratable native secrets, retained native export/import cache files, account-unbound browser vault records, and unbounded encrypted mutation payloads; its 12-check verifier passes. Managed migration `20260726120000` is applied and lint-clean. The managed-test EAS alias and immutable deployment pass all 16 response checks. A live `pnpm audit --prod --json` reports zero findings at every severity and no advisories. | Connected/physical/external gate: run the Docker preservation verifier, validate device-bound storage and cache cleanup on physical devices, repeat the header and database verifiers on production, commission the independent cryptographic/application review, resolve every critical/high finding it identifies, and rerun the production audit for the release candidate. |
...
- `pnpm verify:security`: 12 device-storage, browser-vault, temporary-file, and
  encrypted-payload-bound checks pass
...
- `pnpm verify:connected:acceptance:backend`: migration head `20260726120000`
  ...
  private and provider-qualified release evidence is still pending
