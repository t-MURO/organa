...
| 17 | Installable offline PWA | Manifest, icons, eight static routes, tested waiting-worker discovery/dismissal/activation/fallback, Workbox shell, an external registration bootstrap, 28 artifact/CSP/header checks, 23 precache URLs, an offline drill, and a real same-origin marked-worker replacement drill pass. The stable managed-test EAS alias and immutable deployment pass all 17 live response checks with exact route headers, managed-only bundle/CSP agreement, bounded static caching, and HTTP-cache-independent worker updates. | Promote and repeat against production, then repeat install/update in every supported release browser and installed mode. |
