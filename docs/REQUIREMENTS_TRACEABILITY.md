...
| 2 | Recovery confirmation and trusted-device enrollment | `packages/crypto` tests checked `ORG1` recovery and target-bound `ODA1` transfer; 75 authenticated backend checks include recovery proof, approval, claim, rejection, expiry, and revocation. Native/web identity parsing rejects malformed stored fields, and a successful device-list read erases local private state when the current identity is missing, untrusted, or revoked without weakening genuine offline access. A connected disposable-browser drill directly proved untrusted-device broadcast, erasure/sign-out, and recovery-required reentry. | Separate-device recovery plus malformed/missing-identity drills remain; repeat against the home-server and production deployments. |
