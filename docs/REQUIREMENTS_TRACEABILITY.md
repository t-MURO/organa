...
| --- | --- | --- | --- |
| 1 | Email verification-code account creation | Email-code adapters and the sign-in UI are in `apps/mobile/src/auth`; local email-code contracts and a real local OTP walkthrough pass. The selected managed production project uses `https://organa.expo.app` as its Site URL. Maileroo custom SMTP and both six-digit/15-minute code-only email templates are configured and pass the read-only Management API verifier. User-confirmed live walkthroughs received and accepted the Maileroo-delivered code on both the preview and stable production origins. The client requests and verifies email codes without a redirect URL. Google/GitHub provider discovery, controls, callback exchange, direct dependencies, credential files, and provisioning tooling are absent; the managed providers remain disabled. | Disable Maileroo tracking before accepting any future link-based security flow; the code-only controlled-beta flow does not depend on tracked links. |
