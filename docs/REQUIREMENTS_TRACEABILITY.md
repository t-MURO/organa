...
| 5 | Multiple reminders, snooze presets, and a primary device | Native plans, task/subtask schedules, Web Push schedules, configurable task presets, ownership cache, and primary demotion are implemented; database checks enforce quiet secondary devices. The guarded connected verifier prepares private-channel primary/secondary ownership, target-proof, revocation, and refresh-invalidation evidence. Focus and active-tab reminders expose every saved preset, while the compact native notification surface exposes the first two quick actions. | Pass the connected live-session and rendered-client cleanup drills, exercise physical native actions and Web Push, then repeat against production. |
