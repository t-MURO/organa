...
| 20 | Critical/high security findings resolved | Security design, local abuse verifiers, explicit revocation of direct access to internal database functions, targeted dependency inspection, exact-hash script CSP, generated host-header/cache policy, and a live deployment verifier are documented. A live `pnpm audit --prod --json` of the lockfile-bound 609-package production graph reports zero findings at every severity and no muted advisories. | Connected/external gate: pass the header verifier on the production HTTPS host, commission the independent cryptographic/application review, resolve every critical/high finding it identifies, and rerun the production audit for the release candidate. |
...
- `pnpm build:web`: 26 production artifact/CSP/header checks and 23 precache
  URLs pass
