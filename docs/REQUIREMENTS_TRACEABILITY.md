...
| 10 | Check-In mood, label, reflection, reminder, search, trends | Domain validation/tests and Check-In UI implement one daily entry, mood 1-5, one-word label, optional reflection, 7/30-day trends, search, separate reminders, durable save feedback, and keyed deterministic opaque IDs that preserve same-day multi-device convergence without exposing the date. | Physical/system reminder drill. |
