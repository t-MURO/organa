...
| 7 | Today, priority/time lanes, week/month calendar | `today-screen.tsx`, `planning-calendar.tsx`, and task planning/domain tests implement these views; browser walkthrough covers switching and task placement. A fresh preview directly proves the required Home order places Priority and Time lanes before Quick Capture. A `1920px` desktop drill measured the widened centered Today and standard canvases, while a `390px` compact drill retained mobile navigation with equal client/scroll widths and no horizontal overflow. | Release-device responsive smoke test. |
